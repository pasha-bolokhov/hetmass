...
\end{equation}
	...
	The  components $A_1^i$ and $A_2^i$ are constant and define the twisted masses\footnote{We note that relations \eqref{mGi} assume 
	the $ \ov{\Phi}\, e^{\displaystyle V}\, \Phi $ normalization of the gauge field $ V $ inside the kinetic term. 
	In the case when the normalization $ \ov{\Phi}\, e^{\displaystyle - 2 V} \Phi $ is used, 
	as we do in the dimensional reduction of SQED in Section~\ref{gaugef},
	the relations instead become $ m^k ~=~ A_1^k ~+~ i\, A_2^k $\,.}
